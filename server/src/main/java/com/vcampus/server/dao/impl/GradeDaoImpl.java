...
public class GradeDaoImpl implements GradeDao {
    ...
    public boolean insert(GradeVO grade) throws SQLException {
        String sql = "INSERT INTO grade(student_id, course_code, course_name, usual_score, "
    ...
    public boolean update(GradeVO grade) throws SQLException {
        String sql = "UPDATE grade SET course_name=?, usual_score=?, exam_score=?, final_score=?, "
                + "gpa=?, status=? WHERE student_id=? AND course_code=?";
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public boolean exists(String studentId, String courseCode) throws SQLException {
        String sql = "SELECT COUNT(*) FROM grade WHERE student_id = ? AND course_code = ?";
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public List<GradeVO> listByStudent(String studentId) throws SQLException {
        String sql = "SELECT student_id, course_code, course_name, usual_score, exam_score, "
                + "final_score, gpa, status FROM grade WHERE student_id = ?";
    ...
    public List<GradeVO> listByCourse(String courseCode) throws SQLException {
        String sql = "SELECT student_id, course_code, course_name, usual_score, exam_score, "
                + "final_score, gpa, status FROM grade WHERE course_code = ?";
        List<GradeVO> result = new ArrayList<GradeVO>();
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {

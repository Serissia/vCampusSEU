...
public class CourseSelectionDaoImpl implements CourseSelectionDao {
    ...
    public boolean insert(CourseSelectionVO selection) throws SQLException {
        String sql = "INSERT INTO course_selection(student_id, course_code, select_time, status) "
                + "VALUES (?, ?, ?, ?)";
    ...
    public boolean delete(String studentId, String courseCode) throws SQLException {
        String sql = "DELETE FROM course_selection WHERE student_id = ? AND course_code = ?";
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public boolean exists(String studentId, String courseCode) throws SQLException {
        String sql = "SELECT COUNT(*) FROM course_selection WHERE student_id = ? AND course_code = ?";
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, studentId);
            ps.setString(2, courseCode);
    ...
    public List<CourseVO> listByStudent(String studentId) throws SQLException {
        String sql = "SELECT c.course_code, c.course_name, c.credit, c.teacher_id, c.teacher_name, "
                + "c.capacity, c.selected_count, c.semester, c.class_time, c.location, c.status "
                + "FROM course_selection cs JOIN course c ON cs.course_code = c.course_code "
                + "WHERE cs.student_id = ?";
